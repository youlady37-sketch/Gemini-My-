...
export const SidePanelApp: React.FC = () => {
  ...
  // Инициализация данных и хранилища
  ...
  useEffect(() => {
    ...
    // Слушаем сообщения об изменении вакансий от content-script
  ...
  // Запуск анализа и создание письма
  const handleAnalyzeAndGenerate = async (targetMode: RewriteMode = 'default') => {
    ...
    try {
      // 1. Анализируем вакансию относительно профиля
      ...
      // 2. Создаем персонализированное сопроводительное письмо
  ...
  const handleRewrite = async (mode: RewriteMode) => {
    ...
    try {
      ...
      setErrorMessage('Не удалось переписать текст. Попробуйте еще раз.');
      setAppState('LETTER_READY');
    } finally {
    ...
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 font-sans select-none antialiased">
      ...
      <header className="sticky top-0 z-30 bg-slate-950/95 backdrop-blur border-b border-slate-800/80 px-3.5 py-2.5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-lg bg-gradient-to-tr from-blue-600 to-indigo-500 flex items-center justify-center text-white text-xs font-bold shadow-sm">
            ✨
          </div>
          <div>
            <h1 className="text-xs font-bold tracking-tight text-white flex items-center gap-1.5">
              HH Reply AI
              <span className="text-[10px] font-normal px-1.5 py-0.2 rounded bg-slate-800 text-slate-300">
                v1.0
              </span>
            </h1>
          </div>
        ...
        <nav className="flex items-center gap-1 bg-slate-900 p-0.5 rounded-lg border border-slate-800">
          ...
          >
            ...
            {historyItems.length > 0 && (
              <span className="text-[10px] ml-0.5 opacity-80 font-mono">
                {historyItems.length}
              </span>
            )}
          </button>
      ...
      <main className="flex-1 p-3.5 space-y-3.5 overflow-y-auto">
        ...
        {activeTab === 'profile' && userProfile && (
          <ProfileView profile={userProfile} onReset={handleResetProfile} />
        )}
        ...
        {activeTab === 'assistant' && (
          <>
            ...
            {errorMessage && (
              <div className="bg-red-950/70 border border-red-800/80 rounded-xl p-3 text-xs text-red-200 flex items-start gap-2 shadow-sm">
                ...
                <div className="flex-1">
                  ...
                  <button
                    onClick={() => handleAnalyzeAndGenerate(currentMode)}
                    className="mt-1.5 text-xs text-red-300 underline font-medium hover:text-white"
                  >
                    Попробовать снова
                  </button>
                </div>
            ...
            {/* СОСТОЯНИЕ 1: Не открыта вакансия */}
            {appState === 'NO_VACANCY' && (
              <div className="space-y-4 py-3">
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-center shadow-sm space-y-3">
                  <div className="w-12 h-12 rounded-2xl bg-blue-950/60 border border-blue-800/40 text-blue-400 flex items-center justify-center mx-auto shadow-inner">
                    <Search className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold text-white">
                      Откройте вакансию на hh.ru
                    </h3>
                    <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                      Перейдите на страницу любой вакансии hh.ru, и расширение автоматически распознает требования.
                    </p>
                  </div>
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium py-2 px-3 rounded-xl border border-slate-700 transition flex items-center justify-center gap-1.5"
                    >
                  ...
                  <div className="space-y-1.5">
                    {sampleVacancies.map((sample) => (
                      <button
                        ...
                        className="w-full text-left p-2.5 rounded-lg bg-slate-950 hover:bg-slate-800/80 border border-slate-800/80 hover:border-slate-700 transition group"
                      >
                        <div className="text-xs font-medium text-slate-200 group-hover:text-blue-300 line-clamp-1">
                          {sample.title}
                        </div>
                        <div className="text-[11px] text-slate-400 flex items-center gap-2 mt-0.5">
                          <span className="font-semibold text-slate-300">{sample.company}</span>
                          {sample.salary && <span>• {sample.salary}</span>}
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {/* СОСТОЯНИЯ 2, 3, 4, 5 (когда вакансия определена) */}
            {currentVacancy && appState !== 'NO_VACANCY' && (
              <div className="space-y-3.5">
                ...
                {/* СОСТОЯНИЕ 2: Вакансия найдена — кнопка действия */}
                {appState === 'VACANCY_FOUND' && (
                  <div className="space-y-2.5">
                    <button
                      onClick={() => handleAnalyzeAndGenerate('default')}
                      className="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-semibold rounded-xl shadow-md transition flex items-center justify-center gap-2"
                    >
                ...
                {/* СОСТОЯНИЕ 3: Загрузка / Анализ */}
                {appState === 'ANALYZING' && <LoadingState />}

                {/* СОСТОЯНИЯ 4 и 5: Анализ и готовое письмо */}
                  ...
                  <div className="space-y-3.5">
                    ...
                    {coverLetterText && (
                      <CoverLetter
                        ...
                        onTextChange={(newTxt) => setCoverLetterText(newTxt)}
                        mode={currentMode}
                        charCount={coverLetterText.length}
                      />
                    )}

                    {/* Кнопки переписывания */}
                    <ActionButtons
                    ...
                    {/* Блок аналитики соответствия */}
                    {analysis && (
                      <div className="pt-1">
                        <div className="flex items-center gap-2 mb-2 px-1">
                          <FileCheck className="w-3.5 h-3.5 text-slate-400" />
                          <h4 className="text-xs font-semibold text-slate-300 uppercase tracking-wider">
                            Детали анализа вакансии
                          </h4>
                        </div>
                    ...
                    <div className="pt-2">
                      <button
                        ...
                        className="w-full py-2 px-3 bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-slate-200 text-xs font-medium rounded-xl border border-slate-800 transition flex items-center justify-center gap-1.5"
                      >
